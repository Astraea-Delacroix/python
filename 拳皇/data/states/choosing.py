...
class Choosing(tools._State):
    ...
    def handle_mouse_click(self, pos):
        p1_offset = self.get_clicked_offset(pos, c.P1_CHOOSE_BASE)
        if p1_offset is not None and not self.p1_confirm:
            self.p1_offset = p1_offset
            self.p1_confirm = True
            return

        p2_offset = self.get_clicked_offset(pos, c.P2_CHOOSE_BASE)
        if p2_offset is not None and not self.p2_confirm:
            self.p2_offset = p2_offset
            self.p2_confirm = True


    def get_clicked_offset(self, pos, base):
        for offset in self.pos2Chara.keys():
            rect = pg.Rect(base[0] + offset[0], base[1] + offset[1], 50, 50)
